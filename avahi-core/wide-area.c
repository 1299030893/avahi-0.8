...
static int send_to_dns_server(AvahiWideAreaLookup *l, AvahiDnsPacket *p) {
    ...
    if (a->proto == AVAHI_PROTO_INET) {
        return avahi_send_dns_packet_ipv4(l->fd, AVAHI_IF_UNSPEC, p, NULL, &a->data.ipv4, AVAHI_DNS_PORT);
    } else {
#ifndef DISABLE_IPV6
        return avahi_send_dns_packet_ipv6(l->fd, AVAHI_IF_UNSPEC, p, NULL, &a->data.ipv6, AVAHI_DNS_PORT);
#else
        return -1;
#endif
    }
